...
function tbWhelk:OnUse()
	...
	if (nLevel == 1) then		-- 小海螺
		nAddCount = 10
		szName = "Ốc truyền thanh（Tiểu）"
	elseif (nLevel == 2) then	-- 中海螺
		nAddCount = 100
		szName = "Ốc truyền thanh（Trung）"
	elseif (nLevel == 3) then	-- 大海螺
		nAddCount = 500
		szName = "Ốc truyền thanh（Đại）"
	else
		return 0
	end
	...
	if (nPayCount > 3000) then
		me.Msg("Số lượng câu trò chuyện trên kênh thế giới đã bổ sung quá 3000 câu, không thể sử dụng thêm.")
		return 0
	end

	me.SetTask(TASK_CHAT, 2, nPayCount * 1000 + nFreeCount)
	me.Msg("Bạn sử dụng " .. szName .. ", nhận được " .. nPayCount .. " lần trò chuyện trên kênh thế giới.")
	return 1
end;
