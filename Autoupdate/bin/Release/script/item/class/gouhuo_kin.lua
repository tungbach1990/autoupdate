...
local tbGouhuoItem	= Item:GetClass("firewood_kin");
...
function tbGouhuoItem:IsPickable(nObjId)
	if me.GetKinMember() <= 0 then
		me.Msg("Không phải thành viên tổ đội, không thể đốt lửa trại.")
		return 0;
	end
...
function tbGouhuoItem:PickUp(nX, nY)
	...
	me.Msg("Bạn đã đốt lửa trại, các thành viên tổ đội có thể cùng chia sẻ kinh nghiệm.");
	return	0;
end
...
function tbGouhuoItem:CallGouhuoNpc(nX, nY, nExistentTime, nBaseMultip)
	...
	tbNpc:InitGouHuo(pNpc.dwId, 2,	nExistentTime, 5, 45, nBaseMultip, 0)
