...
local tbItem = Item:GetClass("extrep1")
...
function tbItem:SureUse(nItemId, nFlag)
	...
	if not nFlag then
		...
		Dialog:Say("Dùng đạo cụ này sẽ tăng thêm một ngăn trong rương chứa đồ, nhưng <color=red>mỗi nhân vật chỉ có thể sử dụng 1 lần<color>, dùng chứ?", tbOpt);
		return 1;
	end
	...
	if me.DelItem(pItem) == 1 then
		...
		me.Msg("Chúc mừng ngươi đã mở thêm một ngăn mới.");
	end
	return 1;	
end
