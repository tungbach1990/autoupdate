...
function tbItem:SureUse(nItemId, nFlag)
	...
	if math.mod(math.floor(nTask/(10^(pItem.nLevel-1))), 10) > 0 then
		Dialog:Say("Bạn đã dùng qua đạo cụ mở rộng rương.");
		return 1;
	end
	if not nFlag then
		local tbOpt =
		...
		Dialog:Say("Dùng đạo cụ này sẽ tăng thêm một ngăn trong rương chứa đồ, nhưng <color=red>mỗi nhân vật chỉ có thể sử dụng một lần<color>, bạn muốn dùng chứ?", tbOpt);
		return 1;
	end
